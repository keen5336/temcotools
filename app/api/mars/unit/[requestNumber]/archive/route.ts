...
import { setMarsUnitArchived } from "@/lib/mars/inventory";
...
export async function POST(
  ...
  if (!body || typeof body.archived !== "boolean") {
    return NextResponse.json(
      { ok: false, error: "Request body must include a boolean archived value." },
      { status: 400 }
    );
  }

  const { requestNumber } = await context.params;
  const unit = await setMarsUnitArchived({
    requestNumber,
