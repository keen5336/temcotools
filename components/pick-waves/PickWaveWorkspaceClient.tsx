        ...
        <div className="space-y-6">
          <section className="card bg-base-100 border border-base-200 shadow-sm">
            <div className="card-body gap-5">
              ...
              <ScannerCapture title="Pick Wave Scanning" description="Scanner events are captured anywhere on the one-screen view without opening the keyboard." onScan={handleScan} enabled={!wave.archivedAt} disabledReason="Restore this wave before scanning." count={scannedCount} countLabel="picked" feedback={scanFeedback} result={lastItem ? <div><div className="text-xs uppercase tracking-[0.2em] text-slate-400">Staging Location</div><div className="text-6xl sm:text-8xl font-black leading-none my-3 break-words">{lastResult?.stagingLocation || "UNASSIGNED"}</div><div className="grid grid-cols-2 gap-3 text-sm">{itemFields(lastItem).map(({ label, value }) => <div key={label}><div className="text-xs uppercase text-slate-400">{label}</div><div className="font-semibold break-words">{value || "—"}</div></div>)}</div></div> : lastResult && !lastResult.matched ? <div className="grid h-full place-items-center text-4xl font-black">Not in pick wave</div> : undefined} />
